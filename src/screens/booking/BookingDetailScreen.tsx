...
import {
  formatDate,
  formatTime,
} from '../../functions/formate/DateTimeFormate';
    ...
    <>
      ...
      {isLoading ? (
        ...
        >
          ...
          >
            <Pill
              icon={<Tag size={13} color={Dark.accent} />}
            ...
            <Pill
              ...
              label={`${formatTime(booking.schedule?.startTime)} - ${formatTime(
          ...
          <View className=" mt-7">
            ...
            >
              <InfoRow
                icon={<User size={15} color={Dark.textSecondary} />}
                label="Name"
                value={booking.applicant?.name}
              />
              <InfoRow
                icon={<Building2 size={15} color={Dark.textSecondary} />}
                label="Organization"
                value={booking.applicant?.organization}
              />
              <InfoRow
                icon={<Phone size={15} color={Dark.textSecondary} />}
                label="Mobile"
                value={booking.applicant?.mobile}
              />
              <InfoRow
                icon={<Mail size={15} color={Dark.textSecondary} />}
                label="Email"
                value={booking.applicant?.email}
              />
              <InfoRow
                icon={<MapPin size={15} color={Dark.textSecondary} />}
                label="Address"
                value={booking.applicant?.address}
                last
              />
            </View>
          </View>
          <View className="mt-7">
            ...
            >
              {booking.financial?.charges?.map(
                (
                  c: { label: string; amount?: number; paid?: number },
                  i: number,
                ) => (
                  <InfoRow
                    key={`${c.label}-${i}`}
              ...
              {booking.financial?.units?.map(
                (
                  u: { label: string; perUnit?: number; currentUnit?: number },
                  i: number,
                ) => (
                  <React.Fragment key={`unit-${u.label}-${i}`}>
                    <InfoRow
                      label={`${u.label} — Per Unit`}
                      value={`₹${(Number(u.perUnit) || 0).toLocaleString()}`}
                    />
                    <InfoRow
                      label={`${u.label} — Current Unit`}
                      value={
                        Number(u.currentUnit) > 0
                          ? Number(u.currentUnit).toLocaleString()
                          : '—'
                      }
                    />
                  </React.Fragment>
              ...
              <InfoRow
                label="Total Amount"
                value={`₹${finTotalAmount.toLocaleString()}`}
                bold
              />
              <InfoRow
                label="Total Paid"
                value={`₹${finPaidAmount.toLocaleString()}`}
              />
              <InfoRow
...
const InfoRow = ({
  ...
  <View
    className={`flex-row items-center justify-between ${last ? '' : 'mb-3.5'}`}
  >
